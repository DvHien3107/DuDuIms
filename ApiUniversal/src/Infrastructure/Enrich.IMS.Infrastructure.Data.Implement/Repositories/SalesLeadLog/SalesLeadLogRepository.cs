...
{
    public partial class SalesLeadLogRepository : DapperGenericRepository<SalesLeadLog>, ISalesLeadLogRepository
    {
        private const string Alias = SqlTables.SalesLeadLog;

        public SalesLeadLogRepository(IConnectionFactory connectionFactory)
            : base(connectionFactory)
        {
        }

        public async Task<IEnumerable<SalesLeadLogDto>> GetByCustomerCodeAsync(string CustomerCode)
        {
            var query = SqlScript.SalesLeadLog.GetByCustomerCode;
            var parameters = new DynamicParameters();
            parameters.Add(SqlScript.Parameters.CustomerCode, CustomerCode);
            using (var connection = GetDbConnection())
            {
                return await connection.QueryAsync<SalesLeadLogDto>(query, parameters);
            }
        }
